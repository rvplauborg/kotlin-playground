...
data class Order private constructor(override val _id: EntityId<Order>, val name: OrderName) : DomainEntity<Order>() {
    fun delete() {
        addDomainEvent(OrderDeletedEvent(_id))
    }

    companion object {
        fun create(id: EntityId<Order>, name: OrderName) = Order(id, name).also { it.addDomainEvent(OrderCreatedEvent(it._id)) }
    }
}
